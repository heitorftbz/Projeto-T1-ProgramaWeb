...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $adm_key = trim(htmlspecialchars($_POST['adm_key']));
    ...
    if (empty($nome) || empty($email) || empty($senha) || empty($confirmar_senha) || empty($data_nascimento)) {
        ...
        } else if ($adm_key != "894309234752"){
            $erro = "Chave do Administrador Incorreta!";
        }
        else {
            $senha_criptografada = password_hash($senha, PASSWORD_BCRYPT);

            try {
                $sql = "INSERT INTO adm (usuario, email, senha, data_nascimento) VALUES (:nome, :email, :senha, :data_nascimento)";
    ...
    <div class="container">
        <div class="box">
            <form action="cadastro.php" method="POST">
                ...
                <div class="inputBox">
                    <input type="password" name="adm_key" id="adm_key" class="inputUser" required>
                    <label for="adm_key" class="labelInput">Chave do Administrador</label>
                </div>
